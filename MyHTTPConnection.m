...
- (NSString *)createBrowseableIndex:(NSString *)path
{
	...
	for (NSString *fname in array)
    {
        NSDictionary *jsonDict = [[NSFileManager defaultManager] fileAttributesAtPath:[path stringByAppendingPathComponent:fname] traverseLink:NO];
		//NSLog(@"fileDict: %@", fileDict);
 ...
 * Customize to fit your needs
 **/
- (NSString *)createBrowseableIndex2:(NSString *)path
{
    NSArray *array = [[NSFileManager defaultManager] directoryContentsAtPath:path];
    
    NSMutableString *outdata = [NSMutableString new];
	[outdata appendString:@"<html><head>"];
	[outdata appendFormat:@"<meta http-equiv='Content-Type' content='text/html; charset=utf-8' />", server.name];
	[outdata appendString:@"</head><body>["];
	    for (NSString *fname in array)
    {
        NSDictionary *fileDict = [[NSFileManager defaultManager] fileAttributesAtPath:[path stringByAppendingPathComponent:fname] traverseLink:NO];
		...
		[outdata appendFormat:@"{name: \"%@\" url: ", fname,  [[fileDict objectForKey:NSFileSize] floatValue] / 1024];
...
- (NSString *) mimeForExt: (NSString *) ext
{
	...
	if ([uc isEqualToString:@"Jpg"]) return @"image/jpeg";
	if ([uc isEqualToString:@"m4v"]) return @"image/jpeg";
	if ([uc isEqualToString:@"mov"]) return @"image/mov";
	if ([uc isEqualToString:@"m2v"]) return @"video/m2v";
	if ([uc isEqualToString:@"MOV"]) return @"video/MOV";
	return nil;
}
